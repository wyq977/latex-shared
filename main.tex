...
I choose AOS's preprint version as the defxault, please check
...
Should I use sans-serif fonts in the figures or just serif.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{2.7in}
		\includegraphics{scienceplots/mine.pdf}
		\caption{Custom parameters}
	\end{subfigure}\hfill
	\begin{subfigure}[t]{2.7in}
		\includegraphics{scienceplots/SciencePlots.pdf}
		\caption{SciencePlots}
	\end{subfigure}

	\caption{Custom matplotlib vs SciencePlots}
\end{figure}

I found a python pkg that plots exactly how I like it, \href{https://github.com/garrettj403/SciencePlots?tab=readme-ov-file}{SciencePlots}.
Below is the config I set in matplotlib, as you can see, this is very annoying to change and maintain.
Be sure to set the figure to \verb|constrained| in layout to get the exact size.
...
\begin{verbatim}
    ...
    # --- Sizes and Weights ---
    "font.size": 11,               # Matches \normalsize
    "axes.labelsize": 11,          # Matches \normalsize
    "xtick.labelsize": 9,          # Matches \footnotesize
    "ytick.labelsize": 9,          # Matches \footnotesize
    "legend.fontsize": 9,          # Matches \footnotesize
    
    "lines.linewidth": 1.25,       # Clean visibility
    "axes.linewidth": 0.8,         # Standard LaTeX line weight
    "patch.linewidth": 0.8,
    ...
    "axes.titlepad": 12,           # Matches setpkgattr{title}{skip}{12pt}
\end{verbatim}
...
Spacing difference caused by symbol declaration.
\begin{align*}
	&p(x\mid\theta) \\
	&\PR[p]{x\mid\theta}\\
	&\condPR[p]{x}{\theta}
...
\begin{table}[ht]
	\centering
	\caption{Probability and expectation notation}
	\begin{tabular}{lcccc}
		\toprule
		 & Probability &  Expectation & Prob (line) & Exp (line) \\
		\midrule
		mathrm & $\PR{x}$ & $\ERW[\mathrm{E}]{x}$ & $\condPR{x}{\theta}$ & $\condERW{x}{\theta}$ \\
		mathsf & $\PR[\mathsf{P}]{x}$   & $\ERW[\mathsf{E}]{x}$ & $\condPR[\mathsf{P}]{x}{\theta}$ & $\condERW[\mathsf{E}]{x}{\theta}$ \\
		mathbb & $\PR[\mathbb{P}]{x}$   & $\ERW[\mathbb{E}]{x}$ & $\condPR[\mathbb{P}]{x}{\theta}$ & $\condERW[\mathbb{E}]{x}{\theta}$ \\
		mathit & $\PR[\mathit{P}]{x}$   & $\ERW[\mathit{E}]{x}$ & $\condPR[\mathit{P}]{x}{\theta}$ & $\condERW[\mathit{E}]{x}{\theta}$ \\
		\bottomrule
	\end{tabular}
\end{table}
